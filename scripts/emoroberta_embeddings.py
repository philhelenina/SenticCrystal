#!/usr/bin/env python3
# emoroberta_embeddings_tf.py
# - TensorFlow/Keras 버전 EmoRoBERTa 임베딩 생성
# - train/val/test 한 번에 저장
# - 출력: /.../embeddings/4way/emoroberta/{pooling}/{split}_embeddings.npy
...
# ===== Safe header (TensorFlow only) =====
os.environ["TOKENIZERS_PARALLELISM"] = "false"  # avoid tokenizer locks
...
# ===== Settings =====
MODEL_NAME_OR_PATH = "/Users/helenjeong/Projects/SenticCrystal/saturn_cloud_deployment/src/features/emoroberta-local"  # 로컬 폴더(스크린샷처럼 tf_model.h5 포함)
MAX_LENGTH = 256
BATCH_SIZE = 32
...
tf.config.set_visible_devices([], 'GPU')  # Force CPU-only execution
from transformers import AutoTokenizer, TFAutoModel

print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] TensorFlow initialized (CPU only)")
...
tokenizer = AutoTokenizer.from_pretrained(
    MODEL_NAME_OR_PATH,
    use_fast=False,  # use slow tokenizer to avoid threading issues
