...
# -*- coding: utf-8 -*-
"""
train_turnlevel_k_sweep_new.py - K-sweep with Focal/PolyLoss + class weights
=============================================================================
Based on train_turnlevel_k_sweep.py with added loss function options:
  - CrossEntropy (default)
  - FocalLoss (Lin et al., ICCV 2017)
  - PolyLoss (Leng et al., ICLR 2022)
  - Class weight strategies: inv_freq, sqrt_inv, effective

Usage:
  # Default CE
  python scripts/turn/train_turnlevel_k_sweep_new.py \
    --task 4way --model_tag sentence-roberta --layer avg_last4 --pool mean \
    --models lstm --k_min 0 --k_max 30 --k_step 5

  # Focal Loss
  python scripts/turn/train_turnlevel_k_sweep_new.py \
    --task 4way --model_tag sentence-roberta --layer avg_last4 --pool mean \
    --models lstm --loss focal --focal_gamma 2.0 --k_min 0 --k_max 30 --k_step 5

  # PolyLoss + class weights
  python scripts/turn/train_turnlevel_k_sweep_new.py \
    --task 6way --model_tag sentence-roberta --layer avg_last4 --pool wmean_pos_rev \
    --models lstm --loss poly --poly_epsilon 1.0 --class_weight effective \
...
# PATHS
# ============================================================
HOME = Path(__file__).resolve().parents[2]  # project root
DATA = HOME / "data"
...
class FocalLoss(nn.Module):
    """Focal Loss (Lin et al., ICCV 2017): FL = -alpha(1-pt)^gamma * log(pt)"""
...
class PolyLoss(nn.Module):
    """PolyLoss (Leng et al., ICLR 2022): CE + epsilon * (1 - pt)"""
    def __init__(self, epsilon=1.0, weight=None, label_smoothing=0.0):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        poly_term = (1.0 - pt).mean()
        return ce + self.epsilon * poly_term


def compute_class_weights(y_train, strategy='inv_freq'):
    """
    Compute class weights from training labels.
    Strategies:
      - inv_freq:  w_c = N / (C * n_c)
      - sqrt_inv:  w_c = sqrt(N / (C * n_c))
      - effective: w_c = (1-beta) / (1-beta^n_c), beta=0.9999
    """
...
def build_criterion(args, y_train, device):
    ...
    if args.class_weight:
        cw = compute_class_weights(y_train, strategy=args.class_weight).to(device)
        print(f"  Class weights ({args.class_weight}): {cw.cpu().numpy().round(3)}")

    if args.loss == 'ce':
        return nn.CrossEntropyLoss(weight=cw)
    elif args.loss == 'focal':
        return FocalLoss(gamma=args.focal_gamma, weight=cw)
    elif args.loss == 'poly':
        return PolyLoss(epsilon=args.poly_epsilon, weight=cw,
...
def load_bayesian_hyperparams(bayesopt_dir, layer, pool, model_name):
    ...
    if not filepath.exists():
        print(f"  Bayesian opt file not found: {filename}")
        return None
...
def run_k_experiment(args, k, model_name, label_map,
    ...
    print(f"\n{'---'*24}")
    print(f"K={k:3d} | Model={model_name.upper()} | Loss={args.loss.upper()}"
          + (f" (gamma={args.focal_gamma})" if args.loss == 'focal' else "")
          + (f" (eps={args.poly_epsilon})" if args.loss == 'poly' else "")
          + (f" | CW={args.class_weight}" if args.class_weight else ""))
    print(f"  Data: Train={Xtr_k.shape[0]}, Val={Xva_k.shape[0]}, Test={Xte_k.shape[0]}")

    # Use Bayesian params if available, otherwise defaults
    if bayesian_params:
    ...
    # Build criterion with class weights
    crit = build_criterion(args, ytr_k, device)
    ...
    return {
        ...
        'hyperparams': {
            ...
            'poly_epsilon': args.poly_epsilon if args.loss == 'poly' else None,
            'label_smoothing': args.label_smoothing if args.loss == 'poly' else None,
            'class_weight': args.class_weight or 'none',
        }
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Turn-level K-sweep with Focal/PolyLoss + class weights")
    ...
    # Loss function
    parser.add_argument("--loss", choices=["ce", "focal", "poly"], default="ce",
                       help="Loss function: ce (CrossEntropy), focal (FocalLoss), poly (PolyLoss)")
    parser.add_argument("--focal_gamma", type=float, default=2.0,
                       help="Focal Loss gamma (focusing parameter)")
    parser.add_argument("--poly_epsilon", type=float, default=1.0,
                       help="PolyLoss epsilon (polynomial coefficient)")
    parser.add_argument("--label_smoothing", type=float, default=0.0,
                       help="Label smoothing for PolyLoss CE component")
    parser.add_argument("--class_weight", choices=["inv_freq", "sqrt_inv", "effective", ""],
                       default="", help="Class weight strategy (empty = no weighting)")

    # Training (defaults, overridden by Bayesian opt if available)
    ...
    print(f"\n{'='*80}")
    print(f"TURN-LEVEL K-SWEEP (Focal/PolyLoss)")
    print(f"{'='*80}")
    ...
    print(f"Loss: {args.loss.upper()}"
          + (f" (gamma={args.focal_gamma})" if args.loss == 'focal' else "")
          + (f" (epsilon={args.poly_epsilon})" if args.loss == 'poly' else ""))
    if args.class_weight:
    ...
    def read_split(split):
        csv_path = csv_dir / f"{split}_{args.task}_unified.csv"
    ...
    # Output dir includes loss info
    loss_tag = args.loss
    if args.loss == 'focal':
        loss_tag = f"focal_g{args.focal_gamma}"
    elif args.loss == 'poly':
        loss_tag = f"poly_e{args.poly_epsilon}"
    if args.class_weight:
        loss_tag += f"_cw{args.class_weight}"

    out_tag = f"{args.task}_{args.model_tag}_{args.layer}_{args.pool}_{loss_tag}"
    ...
    if args.use_bayesopt and args.bayesopt_dir:
        ...
        for model_name in args.models:
            ...
            if params:
                ...
                print(f"  No Bayesian params for {model_name}, will use defaults")
        print()
    ...
    n_exp = len(k_values) * len(args.models) * args.seeds
    print(f"{'='*80}")
    print(f"STARTING K-SWEEP: {len(k_values)}K x {len(args.models)}models x {args.seeds}seeds = {n_exp} experiments")
    print(f"{'='*80}\n")
    ...
    for seed in range(args.seeds):
        ...
        for k_idx, k in enumerate(k_values):
            ...
            print(f"K = {k} ({k_idx+1}/{len(k_values)})" +
                  (f" | Seed {seed+1}/{args.seeds}" if args.seeds > 1 else ""))
            print(f"{'='*80}")
            ...
            for model_name in args.models:
                ...
                if wb:
                    log_dict = {
                        'k': k, 'model': model_name, 'seed': seed,
    ...
    # If multiple seeds, compute mean/std
    if args.seeds > 1:
        numeric_cols = [c for c in df.columns
                        if c not in ('model', 'k', 'seed', 'loss', 'class_weight',
                                     'confusion_matrix') and not c.startswith('hp_')]
        agg_df = df.groupby(['model', 'k'])[numeric_cols].agg(['mean', 'std']).reset_index()
        agg_df.columns = ['_'.join(col).strip('_') for col in agg_df.columns.values]
        agg_csv = out_dir / "k_sweep_aggregated.csv"
        agg_df.to_csv(agg_csv, index=False)
        print(f"  {agg_csv}")

    # Use seed=0 results (or mean) for plots
    plot_df = df[df['seed'] == 0] if args.seeds > 1 else df
    plot_comprehensive_analysis(plot_df, label_map, out_dir, args.task)
    ...
    analysis = {
        ...
        'total_experiments': len(all_seed_results),
        'emotions': list(label_map.values()),
        'loss_function': args.loss,
        'class_weight_strategy': args.class_weight or 'none',
    ...
    # Use mean across seeds for analysis
    if args.seeds > 1:
        analysis_df = df.groupby(['model', 'k']).mean(numeric_only=True).reset_index()
    else:
        analysis_df = df

    for emotion in label_map.values():
        recall_col = f'{emotion}_recall'
        if recall_col in analysis_df.columns:
            emotion_data = analysis_df[analysis_df['model'] == analysis_df['model'].iloc[0]]
            ...
            analysis['best_performance_per_emotion'][emotion] = {
                'k': optimal_k, 'recall': best_recall, 'f1': best_f1
            }
            ...
            if emotion in analysis['saturation_params']:
                ...
                print(f"  Saturation tau: {tau:.2f} turns")
