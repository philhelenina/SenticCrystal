...
Input defaults (IEMOCAP 4-way):
  ...
  Audio: data/embeddings/4way/opensmile/eGeMAPSv02/{train,val,test}_filtered.npy

Output default:
  data/embeddings/4way/opensmile_sroberta/cross_modal_attention/avg_last4/wmean_pos_rev/{train,val,test}.npz
...
def _load_text_embeddings(npz_path: Path) -> np.ndarray:
    ...
    if "embeddings" in data:
        ...
        raise KeyError(f"No 'embeddings' key in {npz_path}. Keys={data.files}")
    return _to_token_sequence(text)


def _load_audio_embeddings(npy_path: Path) -> np.ndarray:
    audio = np.load(npy_path, allow_pickle=True).astype(np.float32)
    if audio.ndim == 2:
        return audio[:, None, :]
    if audio.ndim == 3:
        # Common shape in this repo is (N, 1, 88).
        return audio
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--audio-base",
        type=Path,
        default=Path("data/embeddings/4way/opensmile/eGeMAPSv02"),
        help="Base directory for audio *_filtered.npy files",
    )
    parser.add_argument(
        "--out-base",
        type=Path,
        default=Path("data/embeddings/4way/opensmile_sroberta/cross_modal_attention/avg_last4/wmean_pos_rev"),
...
def main() -> None:
    ...
    sample_audio = _load_audio_embeddings(args.audio_base / "train_filtered.npy")
    ...
    for split in splits:
        ...
        audio_path = args.audio_base / f"{split}_filtered.npy"
