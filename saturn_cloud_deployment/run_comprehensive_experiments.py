...
def run_baseline_experiments(config_id):
    ...
    cmd = [
        'python', str(HOME_DIR / 'saturn_cloud_deployment' / 'train_baseline_classifier.py'),
...
def run_turn_experiments(config_id, k_values, models=['lstm', 'mlp']):
    ...
    for k in k_values:
        ...
        for model in models:
            ...
            cmd = [
                'python', str(HOME_DIR / 'saturn_cloud_deployment' / 'train_turn_classifier.py'),
                '--config_id', str(config_id),
                '--k_value', str(k),
                '--model', model,
                '--batch_size', '32',
...
def main():
    ...
    parser.add_argument('--k_values', nargs='+', type=int, default=[2, 4, 6, 8, 10, 15, 20], 
